	...
	*/
	...
	if (isset($_POST['new_schetfactura1'])) {
		...
		$this->load->view("reports/avp",$data);
			...
			{$string = $this->load->view("reports/schetfactura_new114",$data,TRUE);
	...
	function poleznyy_otpusk()
	{
		...
		$this->db->where('period_id',$_POST['period_id']);
		#added
		#$data['otpusk']=$this->db->get('industry.po_ksk');
		#$data['otpusk']=$this->db->get('industry.po_bezksk');
		#added
	...
	function pre_perehod()
	{
		...
		$data['periods'] = $this->db->query(
			"select period.* from industry.period
			 join industry.sprav on sprav.name='current_period'
			 where period.id<sprav.value::INTEGER
			 and period.id >104"
		)->result();
		#end of added
